...
For Cloud Run changes, also run `node --test tests/runner.browser.cjs`. It checks later unlocks, score/bonus UI, pause, rotation, restart, and panel/HUD clipping in both browser engines. The unit suite simulates ten-minute mixed runs with several random seeds at phone and desktop stage widths to check reaction gaps and jump/duck sequences.
...
`nm-run-engine.mjs` contains deterministic movement, collision, obstacle spacing, stars and scoring. Active play time unlocks high birds at 12 seconds, paper planes at 25, balloons at 45, shooting stars at 70, and Rush Hour at 105. The first encounter after each unlock introduces that type; later encounters mix the unlocked pool. Jump low birds and balloons; duck high birds and planes. Speed ramps smoothly from 210 toward 360 canvas pixels per second while recovery intervals shrink toward 0.85 seconds. All encounters share track speed to prevent a fast plane catching a balloon and forcing conflicting actions. Keep the recovery interval after each obstacle's entire width, with room for landing and reacting on phones. Pausing freezes the clock and restart resets progression.

Rendering and collision share `DUCK_HEIGHT` and `obstacleBounds`. Bird flight clearances remain 34 and 58 canvas pixels, measured from the anchored body, with forgiving wing tips. Both bird poses stay anchored around the beak. Planes use a body collider that clears the 36-pixel duck pose; balloons bob by three pixels with their collider following the drawing. High birds and planes carry a low star collectible only while grounded and ducking. Low birds and balloons can carry a star above them. Shooting stars descend diagonally in their own encounter slot, have a fixed pixel trail, and award three stars (75 points); missing one is harmless. A collision cannot award a star.

The transparent sky atlas supplies clouds, birds, the paper plane, and the star core. `balloonArt` draws and caches a small pixel balloon once at load time, with no new asset request. The HUD shows the current level and briefly announces each unlock without interrupting play. The sky changes to violet when shooting stars unlock.
