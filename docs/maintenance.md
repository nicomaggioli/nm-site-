...
The first homepage visit in a tab session opens with a small circular window into the existing work collage. It grows for 520ms, then blends into the existing NM mask from 430–1390ms; after a short settle, the header and hero captions fade in over 450ms. `window.__nmOpening` supplies `active`, `radius` and `morph` to the hero shader in `nm-home-*`; inactive state or morph 1 uses the original mask unchanged. There is no second canvas, image set, layout transform, or scroll lock for this sequence.

`js/nm-opening-boot.js` and `css/nm-opening.css` are inlined by `python3 tools/build-opening.py`; `js/nm-opening.js` is deferred. The bootstrap prevents a header flash before hydration and has its own 5-second fail-open deadline if the controller does not load. Input cancellation is in that bootstrap too, so scrolling, touching, clicking or pressing a key always releases the interface. The controller waits one frame after the ready queue to let the authored collage mount, then waits for decoded images, the first rendered mask frames and the phone collage cache. The circle and morph retain a soft mask edge and land on the original logo mask exactly. It animates scalar shader inputs only, stops its animation loop/listeners at completion, and never stops or starts Lenis. The running animation has a separate 2.3-second watchdog.

Session key `nm-opening-seen-v1` prevents replay on reload or return navigation. Any hash/deep link, Back/Forward entry, reduced motion, Save-Data or initially hidden document skips it. Later input, restored scrolling, width changes, hidden tabs, pagehide and persisted pageshow cancel it. Storage failures remain nonfatal. Narrow browser toolbar height changes do not cancel or restart the animation. A missing texture can still prevent the existing WebGL logo from drawing, but cannot leave the header hidden or prevent navigation/scrolling.

After editing opening source or CSS, run its generator before other metadata/fallback generators and refresh normal CSS/JS content versions. Use `node --test tests/opening.browser.cjs` in both browser engines to check cold completion, same-session returns, cancellation, reduced motion, deep links, and delayed/failed assets. The browser's GPU and a physical phone can still differ; desktop emulation is not a physical iPhone performance measurement.
