...
Touch devices and viewports up to 900px use a Game Boy-style control deck: exactly three buttons, with a round red B/Duck button on the left, Pause in the middle, and a round A/Jump button on the right. There is no directional pad or duplicate duck button. Portrait phones reserve room below the field for both thumbs; short landscape uses smaller (at least 48px) action buttons and a 900×340 logical stage. Short portrait phones hide secondary copy to keep the controls visible. Safe-area padding keeps the deck away from screen corners. Desktop keyboard controls and the compact button row are unchanged. Pointer capture handles release outside a button; source sets preserve simultaneous holds; cancellation, pause, blur, restart and exit clear inputs and visual pressed states.
